...
class _ChatInputFieldState extends State<ChatInputField> {
  ...
  void _startUploading(File file) {
    if (!_uploadingFiles.contains(file)) {
      ...
      // إضافة timeout تلقائي لإخفاء الـ indicator بعد 30 ثانية
      Future.delayed(const Duration(seconds: 30), () {
        if (mounted && _uploadingFiles.contains(file)) {
          print('⏰ Upload timeout - hiding indicator for file: ${file.path}');
  ...
  Widget _buildUploadingIndicator() {
    // تحقق إضافي للتأكد من أن الـ indicator يظهر فقط عندما يكون هناك ملفات قيد الرفع
              ...
              BlocListener<UploadAttachmentsViewModel, UploadAttachmentsViewModelStates>(
                listener: (context, state) {
                  ...
                  if (state is UploadAttachmentsViewModelStatesUploading) {
                    print('📤 Uploading file: ${state.file.path} - ${(state.progress * 100).toStringAsFixed(0)}%');
                    ...
                    if (state.file != null) {
                      _finishUploading(state.file!);
                      print('🗑️ Removed file from uploading: ${state.file!.path}');
                    } else {
                      // إذا file كان null، نظف جميع الملفات
                      _resetUploadState();
                      print('🔄 Reset upload state due to null file');
                    }
                    ...
                    // لا تفعل شيئاً هنا، انتظر حالة Uploading
                  }
                },
              ),
            ],
